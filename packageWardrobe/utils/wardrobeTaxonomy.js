/**
 * 衣橱类型/品类 taxonomy（添加衣物 AI 识别与页面共用）
 */
...
function getSubCategories(typeId) {
  return (SUB_CATEGORIES[typeId] || []).slice()
}

function getCategoryTabsWithCustom(app) {
  const tabs = CATEGORY_TABS.slice()
  const customTypes = app && app.getCustomTypes ? app.getCustomTypes() : []
...
function getCategoryOptionsWithCustom(typeId, app) {
  const base = getSubCategories(typeId)
  if (!app || !app.getPrivateSubConfig) return base
  const config = app.getPrivateSubConfig() || {}
...
/** 将 AI 返回结果映射为合法 typeId / categoryId */
function resolveClassification(raw, app) {
  if (!raw || typeof raw !== 'object') return null

  const globalHit = findByCategoryNameGlobally(raw.categoryName || raw.categoryId)
  if (globalHit) {
    const opts = getCategoryOptionsWithCustom(globalHit.typeId, app)
    return {
      itemTypeId: globalHit.typeId,
      itemType: globalHit.typeName,
      itemCategoryId: globalHit.categoryId,
      itemCategory: globalHit.categoryName,
      categoryOptions: opts,
      aiSummary: raw.summary ? String(raw.summary).trim() : '',
  ...
  const opts = getCategoryOptionsWithCustom(typeHit.typeId, app)
  const inOpts = opts.find((o) => o.id === catHit.id)
  return {
    itemTypeId: typeHit.typeId,
    itemType: typeHit.typeName,
    itemCategoryId: inOpts ? catHit.id : (opts[0] && opts[0].id) || catHit.id,
    itemCategory: inOpts ? catHit.name : (opts[0] && opts[0].name) || catHit.name,
    categoryOptions: opts,
    aiSummary: (raw && raw.summary) ? String(raw.summary).trim() : '',
    aiConfidence: typeof raw.confidence === 'number' ? raw.confidence : null
  }
}

/** 按衣物类型与场景返回抠图参数（支持多引擎） */
...
function getMattingEngineRecommendation(typeId, sceneInfo) {
  ...
  const aiEngine = String((sceneInfo && sceneInfo.mattingEngine) || '').toLowerCase()

  if (aiEngine === 'goods' || aiEngine === 'general') {
    return {
...
function buildTaxonomyPromptText() {
  const lines = ['可选类型与品类（必须从下列 id 中选择）：']
